...
def process_image(image_bytes, size=128, padding_ratio=0.10):
...
def run_theme(theme, args):
    ...
    return success, already, fail, cost
...
def main():
    ...
    if args.recipes:
        ...
        for icon in tqdm(to_generate, desc="Generating"):
            result = generate_icon(icon, output_dir, style, size=400)
            if result == "ok":
                success += 1
                time.sleep(SLEEP)
            else:
    ...
    if args.list:
        for theme in themes:
            ...
            filter_icons(icons, args)
        return

    # Compute total cost across all themes
    total_todo = 0
    for theme in themes:
        saved = args.theme
        args.theme = theme
        ...
        icons = filter_icons(icons, args)
        args.theme = saved
        total_todo += sum(1 for i in icons if not (output_dir / i["filename"]).exists())

    if total_todo == 0:
        print("Nothing to do.")
        return

    total_cost = total_todo * COST_PER_IMAGE
    print(f"💰 Total estimated cost: ~${total_cost:.2f} ({total_todo} icons × ${COST_PER_IMAGE})")
    confirm = input("Continue? [y/N] ").strip().lower()
    if confirm != "y":
        print("Aborted.")
        return
