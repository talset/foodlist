...
def process_recipe_image(pil_image, size=400):
...
def generate_icon(client, icon, output_dir, style, size=128):
    ...
    for attempt in range(RETRIES):
        try:
            ...
            output_path.write_bytes(process_recipe_image(image, size=size) if size > 128 else process_image(image, size=size))
            return "ok"
